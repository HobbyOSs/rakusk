...
## 課題と次のステップ
- `JMP FAR [ESP+4]` 等の FAR ジャンプのパース/バリアント選択が壊れている可能性があり、`day15` 以降のテストでエラーが発生している。これを修正する。
- `day03_harib00j.t` 等の 32 ビットモードのテストで、ジャンプ命令の推定サイズ変更による副作用（リロケーション情報の不一致等）が発生している可能性がある。
- 全テストがパスするようにデグレを修正する。
